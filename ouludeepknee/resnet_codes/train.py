...
from ouludeepknee.resnet_codes.dataset import KneeGradingDataset, LimitedRandomSampler
from ouludeepknee.resnet_codes.train_utils import train_epoch, adjust_learning_rate
from ouludeepknee.resnet_codes.val_utils import validate_epoch
from ouludeepknee.resnet_codes.augmentation import (CenterCrop, CorrectGamma, Jitter, Rotate,
                                                    CorrectBrightness, CorrectContrast)


SNAPSHOTS_KNEE_GRADING_RESNET = os.path.abspath(os.path.join(
    os.path.dirname(__file__), 'snapshots_knee_grading_resnet'))


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset',  default='../../KL_data')
    parser.add_argument('--snapshots',  default=SNAPSHOTS_KNEE_GRADING_RESNET)
    ...
    net.fc = nn.Sequential(nn.Dropout(args.drop), nn.Linear(512,5))
